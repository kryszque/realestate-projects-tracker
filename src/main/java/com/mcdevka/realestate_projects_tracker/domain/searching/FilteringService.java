...
public class FilteringService {
    ...
    public GlobalSearchingResultDTO filterSearch(GlobalSearchingResultDTO sourceData, FilteringCriteria criteria) {
        ...
        if (!CollectionUtils.isEmpty(criteria.getFilteredTagsNames())) {
            projects = filterProjectsByTags(projects, criteria.getFilteredTagsNames());
            // pillars = filterPillarsByTags(pillars, criteria.getFilteredTagsNames());
            items = filterItemsByTags(items, criteria.getFilteredTagsNames());
        }

        return new GlobalSearchingResultDTO(projects, pillars, items);
    }

    private List<Item> filterItemsByTags(List<Item> items, List<String> tagNamesToFind) {
    ...
    private List<Project> filterProjectsByTags(List<Project> projects, List<String> tagNamesToFind) {
